...
async function connectToPort() {
   ...
   const response = await fetch("/portConnection", {
    ...
    for (let i = 0; i < data.PIDList.length; i++) {
        ...
        label.innerHTML = `<input type="checkbox" value="${data.PIDList[i][0]}" data-name="${data.PIDList[i][1]}"> ${data.PIDList[i][0]} - ${data.PIDList[i][1]}`;
...
async function PIDSubmission(n) {
    ...
    if (n == 0) {
        ...
        for (let i = 0; i < inputs.length; i++) {
            if (inputs[i].checked) {
                console.log(inputs[i].value, "->", inputs[i].dataset.name);
                SELECTED_PIDS.push(inputs[i].value);
                PID_NAMES[inputs[i].value] = inputs[i].dataset.name;
            }
        }
        ...
        const namesParam = encodeURIComponent(JSON.stringify(PID_NAMES));
        window.location.href = `/tracking?pids=${pidsParam}&names=${namesParam}`;
    }
}