...
class OBD:
    ...
    #         response = self.DEVICE.read(self.DEVICE.in_waiting).decode(errors="ignore").replace('\r', '').replace('>', '').strip()
    #         split_response = response.split(' ')
    #         print(f"command={command}, response={response!r}, split={split_response}")  # add this
    #         if len(split_response) > 2 and split_response[1] in self.COMMAND.FUNCTIONS:
    #             res = self.COMMAND.FUNCTIONS[split_response[1]](response)
    #             self.LOG.logResponse(command, response)
    #             return {'command': command, 'response': res, 'timestamp': datetime.now().strftime("%I:%M:%S %p")}
    ...
    def singleCommand(self, command):
        try:
            ...
            start = time.time()
            buffer = b''
            while b'>' not in buffer and (time.time() - start) < 1.0:
                if self.DEVICE.in_waiting > 0:
                    chunk = self.DEVICE.read(self.DEVICE.in_waiting)
                    print(f"CHUNK: {chunk!r}")   # add this
                    buffer += chunk
                else:
                    time.sleep(0.005)

            print(f"FULL BUFFER: {buffer!r}")   # and this
            response = buffer.decode(errors="ignore").replace('\r', '').replace('>', '').strip()
            print(f"RAW RESPONSE: {response!r}")

            split_response = response.split(' ')
            print(f"SPLIT: {split_response}")
            ...
            if len(split_response) > 2 and split_response[1] in self.COMMAND.FUNCTIONS:
                ...
                print("CONDITION FAILED — falling through to None")
